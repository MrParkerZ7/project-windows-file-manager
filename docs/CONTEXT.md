...
| 2026-09-04 | **First design-time record.** Four rounds of shell redesign on the Broadsheet design system were captured as a design canvas and vendored into `docs/design/` — the fifth documentation home, empty until now because every prior feature was specced by reverse-engineering the code. Option **3a** (IDE-standard tool-window frame: 34px title bar, underlined editor tabs, a 260/1/1fr/1/360 tool-window grid, one action bar, one status bar) was adopted and planned as epic `G-001`; option **4a** (light/dark theme switch, colour-as-meaning) was explored and deferred. | DESIGN-001 |
