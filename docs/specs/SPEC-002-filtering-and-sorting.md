...
9. **Neither numeric box restricts typed input.** The repo's digit-only input filter (`Views/Support/NumericInputFilter.HandleDigitsOnly`, forwarded from `FoldersScreen.IntegerTextBox_PreviewTextInput` and `FolderActionPanel.IntegerTextBox_PreviewTextInput` since T-002) is wired to the folder-search depth box and the link-siblings layer box only. `MinFileSizeText` is a `string` parsed leniently (rule 3); `MinDuplicateCount` is an `int` binding, so non-numeric text fails WPF conversion and the property retains its previous value.
