...
| Column header click | `FoldersScreen.FolderResults_ColumnHeaderClick` | code-behind sorting on the default `CollectionView` |
...
19. **Sorting is code-behind.** `FoldersScreen.FolderResults_ColumnHeaderClick` maps the header text to a sort property, overriding the display binding for two columns so formatted strings do not sort lexicographically: `"Size"` → `TotalSize`, `"Full Path"` → `FullPath`; other columns fall back to the column's `DisplayMemberBinding` path. Clicking the same header again toggles direction, and the header text is rewritten with a ` ▲` / ` ▼` suffix (all headers are stripped of the suffix first).
