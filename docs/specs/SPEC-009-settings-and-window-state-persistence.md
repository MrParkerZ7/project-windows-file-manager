...
- **The persistence layer's UI paths are untested.** `SettingsService` is fully covered, but `MainViewModel`'s save/load/snapshot/apply/profile code and `MainWindow`'s geometry code are `[ExcludeFromCodeCoverage]` ([ADR-005](../adr/ADR-005-coverage-enforcement-coverlet-msbuild.md)).
