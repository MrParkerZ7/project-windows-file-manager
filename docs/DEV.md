...
| CI coverage gate fails intermittently on an unchanged commit, one module at **0%** | Known flake in coverlet.msbuild instrumentation on the CI runner, **not** reproducible locally. Proven 2026-09-02: commit `da50cf6` failed, then passed on a bare re-run with zero changes. Green ×2 then red ×1 across three docs-only commits. | Re-run the job (`gh run rerun <id> --failed`). If it becomes frequent, move the gate to `coverlet.collector` + a `--collect` run, which does not depend on msbuild instrumentation ordering. Do not lower `<Threshold>`. |
